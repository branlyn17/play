            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <div class="flex items-center gap-3 text-sm text-[color:var(--admin-muted)]">
                        ...
                        <span>{{ trans('admin.template_categories.edit.breadcrumb') }}</span>
                    </div>
                    <h1 class="mt-4 text-4xl font-semibold tracking-tight text-[color:var(--admin-text)] sm:text-[2.7rem]">{{ trans('admin.template_categories.edit.title') }}</h1>
                    <p class="mt-3 max-w-3xl text-base leading-8 text-[color:var(--admin-text-soft)]">{{ trans('admin.template_categories.edit.subtitle') }}</p>
                </div>
                ...
                >
                    {{ trans('admin.template_categories.edit.back') }}
                </a>
            </div>
        </section>

        @include('admin.template-categories._form', [
            'action' => route('admin.template-categories.update', array_merge(['templateCategory' => $category], request()->has('lang') ? ['lang' => request()->query('lang')] : [])),
            'method' => 'PUT',
            'submitLabel' => trans('admin.template_categories.edit.submit'),
        ])
    </div>
@endsection
