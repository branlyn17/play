            ...
            <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div>
                    <div class="flex items-center gap-3 text-sm text-[color:var(--admin-muted)]">
                        ...
                        <span>{{ trans('admin.templates.edit.breadcrumb') }}</span>
                    </div>
                    <h1 class="mt-4 text-4xl font-semibold tracking-tight text-[color:var(--admin-text)] sm:text-[2.7rem]">{{ trans('admin.templates.edit.title') }}</h1>
                    <p class="mt-3 max-w-3xl text-base leading-8 text-[color:var(--admin-text-soft)]">{{ trans('admin.templates.edit.subtitle') }}</p>
                </div>
        ...
        <form method="POST" action="{{ route('admin.templates.update', array_merge(['template' => $template], $langQuery)) }}" enctype="multipart/form-data" class="space-y-5">
                ...
                <div class="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
                    <div class="space-y-5">
                        <div>
                            ...
                            <p class="mt-2 text-sm leading-7 text-[color:var(--admin-text-soft)]">{{ trans('admin.templates.edit.base_help') }}</p>
                        </div>
                        ...
                        <div class="grid gap-4 sm:grid-cols-2">
                            <label class="block">
                                ...
                                <input value="{{ $template->code }}" disabled class="w-full rounded-[1.1rem] border border-[color:var(--admin-border)] bg-[color:var(--admin-surface-soft)] px-4 py-3 text-sm text-[color:var(--admin-text-soft)] outline-none">
                            </label>
                            ...
                            <label class="block">
                                ...
                                <select name="category_key" class="w-full rounded-[1.1rem] border border-[color:var(--admin-border)] bg-[color:var(--admin-surface-soft)] px-4 py-3 text-sm text-[color:var(--admin-text)] outline-none">
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->key }}" @selected($selectedCategory === $category->key)>
                            ...
                            <label class="block">
                                ...
                                <input type="number" min="1" name="sort_order" value="{{ old('sort_order', $template->sort_order) }}" class="w-full rounded-[1.1rem] border border-[color:var(--admin-border)] bg-[color:var(--admin-surface-soft)] px-4 py-3 text-sm text-[color:var(--admin-text)] outline-none">
                            </label>
                            ...
                            <label class="block">
                                ...
                                <select name="catalog_accent" class="w-full rounded-[1.1rem] border border-[color:var(--admin-border)] bg-[color:var(--admin-surface-soft)] px-4 py-3 text-sm text-[color:var(--admin-text)] outline-none">
                                    @foreach ($colorTokens as $token)
                                        <option value="{{ $token }}" @selected($selectedAccent === $token)>{{ ucfirst($token) }}</option>
                                    @endforeach
                                </select>
                            </label>
                        </div>
                        ...
                        <label class="block">
                            ...
                            <textarea name="catalog_background" rows="4" class="w-full rounded-[1.1rem] border border-[color:var(--admin-border)] bg-[color:var(--admin-surface-soft)] px-4 py-3 text-sm text-[color:var(--admin-text)] outline-none">{{ old('catalog_background', $designTokens['catalog_background'] ?? '') }}</textarea>
                        </label>
                        ...
                        <div class="grid gap-4 sm:grid-cols-3">
                            <label class="flex items-center gap-3 rounded-[1.1rem] border border-[color:var(--admin-border)] bg-[color:var(--admin-surface-soft)] px-4 py-3 text-sm text-[color:var(--admin-text)]">
                                <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $template->is_active)) class="h-4 w-4 rounded border-white/20 bg-transparent">
                                <span>{{ trans('admin.templates.create.fields.is_active') }}</span>
                            </label>
                            <label class="flex items-center gap-3 rounded-[1.1rem] border border-[color:var(--admin-border)] bg-[color:var(--admin-surface-soft)] px-4 py-3 text-sm text-[color:var(--admin-text)]">
                                <input type="checkbox" name="is_featured" value="1" @checked(old('is_featured', $template->is_featured)) class="h-4 w-4 rounded border-white/20 bg-transparent">
                                <span>{{ trans('admin.templates.create.fields.is_featured') }}</span>
                            </label>
                            <label class="flex items-center gap-3 rounded-[1.1rem] border border-[color:var(--admin-border)] bg-[color:var(--admin-surface-soft)] px-4 py-3 text-sm text-[color:var(--admin-text)]">
                                <input type="checkbox" name="is_premium" value="1" @checked(old('is_premium', $template->is_premium)) class="h-4 w-4 rounded border-white/20 bg-transparent">
                    ...
                    <div class="space-y-5">
                        <div>
                            ...
                            <p class="mt-2 text-sm leading-7 text-[color:var(--admin-text-soft)]">{{ trans('admin.templates.edit.replace_help') }}</p>
                        </div>
            ...
            <div class="flex justify-end">
                <button type="submit" class="inline-flex h-12 cursor-pointer items-center justify-center rounded-[1.1rem] bg-[color:var(--admin-primary)] px-5 text-sm font-semibold text-white shadow-[0_18px_34px_rgba(79,124,255,0.22)] transition hover:brightness-110">
                    {{ trans('admin.templates.edit.submit') }}
                </button>
            </div>
        </form>
    </div>
